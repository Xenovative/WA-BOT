...
class ChatHandler {
   ...
   * @param {string} [platform] - Platform identifier ('telegram', 'whatsapp', etc.) - optional for native IDs
   */
  addMessage(chatId, roleOrMessage, content, platform) {
    ...
    // Ensure chatId is a string and use it directly as the storage key
      ...
      if (fs.existsSync(chatFile)) {
        try {
          ...
          if (!Array.isArray(existingMessages)) {
            console.warn(`[ChatHandler] Existing chat file for ${nativeChatId} is not an array, initializing new array`);
            const fileContent = fs.readFileSync(chatFile, 'utf8');
            existingMessages = JSON.parse(fileContent);
            if (!Array.isArray(existingMessages)) {
