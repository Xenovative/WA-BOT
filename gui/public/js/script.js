...
// Handle KB document upload
async function handleKBUpload(files) {
  if (!files || files.length === 0) {
    showToast('Please select documents to upload', 'warning');
    return;
  }
  ...
  for (let i = 0; i < files.length; i++) {
    ...
    if (!allowedTypes.includes(fileExt)) {
      showToast(`Invalid file type: ${file.name}. Allowed types: PDF, TXT, MD, DOC, DOCX`, 'error');
      return;
    }
  ...
  try {
    ...
    console.log('[KB] Uploading documents...');
    ...
    if (result.success) {
      let message = `Successfully uploaded ${files.length} document(s)`;
      showToast(message, 'success');
      
      // Refresh KB status and documents list
      setTimeout(() => {
        loadKBStatus();
        loadKBDocuments();
      }, 1000);
    ...
    console.error('[KB] Error uploading documents:', error);
    showToast(`Upload error: ${error.message}`, 'error');
  } finally {
    // Reset button state
    const uploadBtn = document.getElementById('upload-kb-btn');
...
// Load KB status information
async function loadKBStatus() {
  try {
    console.log('[KB] Loading status...');
    ...
    if (result.success) {
      ...
      const documentCount = document.getElementById('kb-document-count');
      const totalDocs = document.getElementById('kb-total-docs');
      const vectorStatus = document.getElementById('kb-vector-status');
      const lastUpdated = document.getElementById('kb-last-updated');
      
      if (result.enabled) {
        statusBadge.className = 'badge bg-success me-2';
        statusBadge.textContent = 'Enabled';
        statusText.textContent = 'Knowledge Base is active';
      } else {
        statusBadge.className = 'badge bg-warning me-2';
        statusBadge.textContent = 'Disabled';
        statusText.textContent = 'Knowledge Base is disabled';
      }
      
      documentCount.textContent = `${result.documentCount || 0} documents`;
      totalDocs.textContent = result.documentCount || 0;
      vectorStatus.textContent = result.vectorStore ? 'Initialized' : 'Not initialized';
      lastUpdated.textContent = result.lastUpdated || 'Never';
      
    } else {
      console.error('[KB] Failed to load status:', result.error);
    }
    
  } catch (error) {
    console.error('[KB] Error loading status:', error);
  }
}

// Load KB documents list
async function loadKBDocuments() {
  try {
    ...
    const tbody = document.getElementById('kb-documents-body');
        ...
        <tr>
          <td>
            <div class="d-flex align-items-center">
              ...
              <div>
                <div class="fw-bold">${escapeHtml(doc.name || doc.filename || 'Unknown')}</div>
                <small class="text-muted">${escapeHtml(doc.id || '')}</small>
              </div>
            </div>
          </td>
          <td>
            <span class="badge bg-light text-dark">${escapeHtml(doc.type || 'Unknown')}</span>
          </td>
          <td>${formatFileSize(doc.size || 0)}</td>
          <td>
            <span class="badge bg-info">${doc.chunks || 0}</span>
          </td>
          <td>
            <small class="text-muted">${formatDate(doc.createdAt || doc.addedAt)}</small>
          </td>
          <td>
            <button class="btn btn-sm btn-outline-danger" onclick="deleteKBDocument('${doc.id}')" title="Delete Document">
              <i class="bi bi-trash"></i>
            </button>
          </td>
        </tr>
      ...
      tbody.innerHTML = `
        <tr>
          <td colspan="6" class="text-center py-4 text-muted">
            ...
            <div class="mt-2">No documents found</div>
    ...
    console.error('[KB] Error loading documents:', error);
    const tbody = document.getElementById('kb-documents-body');
    tbody.innerHTML = `
      <tr>
        <td colspan="6" class="text-center py-4 text-danger">
          <i class="bi bi-exclamation-triangle"></i>
          <div class="mt-2">Error loading documents</div>
...
async function deleteKBDocument(documentId) {
  ...
  try {
    console.log(`[KB] Deleting document: ${documentId}`);
    ...
    console.error('[KB] Error deleting document:', error);
